...
def execute():
    try:
        ...
        if req.operation == "addInstances":
            ...
            url = "http://localhost:55011/rest/addInstances"
            headers = {
            ...
            url = "http://localhost:55012/rest/changeLBWeights"
            headers = {
                'Content-Type': 'application/json'
            }
            request_body = {
            ...
            URL = "http://localhost:55012/rest/changeProperty"
            headers = {
                'Content-Type': 'application/json'
            }
            request_body = {
            ...
            url = "http://localhost:55011/rest/removeInstance"
            headers = {
                'Content-Type': 'application/json'
            }
            request_body = {
...
def fetch_system_architecture():
    url = "http://localhost:55010/rest/systemArchitecture"
...
def fetch_instance_configuration(serviceName, implementationId):
    url = "http://localhost:55010/rest/service/{}/configuration?implementationId={}".format(serviceName,
                                                                                            implementationId)
    try:
        response = requests.get(url)
        response.raise_for_status()
...
def fetch_service_snapshot(serviceName):
    url = "http://localhost:55010/rest/service/{}/snapshot".format(serviceName)
    try:
        response = requests.get(url)
        response.raise_for_status()
